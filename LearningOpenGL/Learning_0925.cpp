...
int main()
{
   ...
   // ------------------------------------------------------------------
	...
	{
		std::cout << __LINE__ << " : Failed to load wall.jpg" << std::endl;
	}
	...
	{
		std::cout << __LINE__ << " : Failed to load awesomeface.png" << std::endl;
	}
	stbi_image_free(data);
	...
	projection = glm::perspective(glm::radians(45.0f), (float)600 / 800, .1f, 100.0f);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		view = glm::lookAt(glm::vec3(camX, .0f, camZ), glm::vec3(.0f), glm::vec3(.0f, 1.0f, .0f));
